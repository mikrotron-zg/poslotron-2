        ...
        <fo:table-cell>
          <fo:block>${uiLabelMap.CommonTo}: </fo:block>
            <#if billingAddress??>
                ...
                <fo:block font-family="NotoSans-Bold">${billToPartyNameResult.fullName?default(billingAddress.toName)?default("Billing Name Not Found")}</fo:block>
                ...
                <#if billToPartyTaxId?has_content>
                    <fo:block space-before="2mm">${uiLabelMap.PartyTaxId}: ${billToPartyTaxId}</fo:block>
                </#if>
            <#else>
