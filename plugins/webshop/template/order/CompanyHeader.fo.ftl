...
<fo:block text-align="left">
    <#if logoImageUrl?has_content>
        <fo:external-graphic src="<@ofbizContentUrl>${logoImageUrl}</@ofbizContentUrl>" 
            overflow="hidden" height="40px" content-height="scale-to-fit" content-width="50mm"/>
    </#if>
</fo:block>

<fo:block font-size="8pt" color="#3f3f3f">
        ...
        <fo:list-block provisional-distance-between-starts="22mm">
            ...
            <#if phone??>
                <fo:list-item>
                    ...
                    <fo:list-item-body start-indent="body-start()">
                        <fo:block>
                            <#if phone.countryCode??>+${phone.countryCode} </#if><#if phone.areaCode??>${phone.areaCode} </#if>${phone.contactNumber!}
                        </fo:block>
                    </fo:list-item-body>
                </fo:list-item>
            </#if>
            ...
            <#if sendingPartyTaxId == "43227166836">
                ...
                <fo:list-item>
                    <fo:list-item-label>
                        <fo:block>MBS 080923147</fo:block>
                    </fo:list-item-label>
                    <fo:list-item-body start-indent="body-start()">
                        <fo:block>Trgovački sud u Zagrebu</fo:block>
                    </fo:list-item-body>
                </fo:list-item>
                <fo:list-item>
                    <fo:list-item-label>
