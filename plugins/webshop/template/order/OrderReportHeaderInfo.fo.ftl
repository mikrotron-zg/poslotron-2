...
<#escape x as x?xml>
  <fo:table table-layout="fixed" font-size="10pt">
    ...
    <fo:table-row>
      <fo:table-cell>
          <fo:block number-columns-spanned="2" font-size="14pt" font-family="NotoSans-Bold">
            ${orderHeader.getRelatedOne("OrderType", false).get("description",locale)}
          </fo:block>
      </fo:table-cell>
    </fo:table-row>
    ...
    <fo:table-row>
      ...
      <fo:table-cell><fo:block font-family="NotoSans-Bold">${currentStatus.get("description",locale)}</fo:block></fo:table-cell>
    </fo:table-row>
